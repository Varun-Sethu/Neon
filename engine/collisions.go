...
func ComputeContactManifold(poly_a, poly_b entities.Polygon) ContactManifold {
	...
	if perb_b >= perp_a {
		reference_polygon, reference_face, incident_polygon, incident_face = poly_b, edge_candidate_b, poly_a, edge_candidate_a
		mtv = mtv.Scale(-1.0)
	}
	...
	contact_points, point_depths := polygon_clip(incident_polygon, reference_polygon, incident_face, reference_face) // note that contact points are in "world-coordinates" aka. Relative to the world origin
	...
	return ContactManifold{
		...
		IncidentFace: incident_face,
		ReferenceFace: reference_face,

		MTV: mtv,
		ContactCount: len(contact_points),
		CollisionPoints: contact_points,
		ContactDepths: point_depths,
	}
}
...
func polygon_clip(incident_poly, reference_poly entities.Polygon, incident_face, reference_face []int) ([]math.Vector2D, []float64) {
	...
	return math.LiesBehindLine(incident_face_edge[:], reference_face_edge, math.ComputeOutwardsNormal(
		reference_face_edge[0], reference_face_edge[1], reference_poly.State.CentroidPosition))
}
